...
async function muatDataPortfolioFirebase() {
    ...
    try {
        // PERBAIKAN 1: Mengubah db2.ref menjadi db.ref agar sinkron dengan Firebase asli kamu
        ...
        // PERBAIKAN 2: Mengubah db2.ref menjadi db.ref untuk mengambil data saldo investasi
        ...
        if(document.getElementById('portfolio-jumlah-saham')) document.getElementById('portfolio-jumlah-saham').innerText = totalSaham.toLocaleString('id-ID') + " Lot";
        if(document.getElementById('portfolio-value-saham')) document.getElementById('portfolio-value-saham').innerText = "Rp " + valueInvestasiSaham.toLocaleString('id-ID');
        
        if(document.getElementById('portfolio-jumlah-cubecoin')) document.getElementById('portfolio-jumlah-cubecoin').innerText = totalCubeCoin.toLocaleString('id-ID') + " CUBE";
        if(document.getElementById('portfolio-value-cubecoin')) document.getElementById('portfolio-value-cubecoin').innerText = "Rp " + valueInvestasiCube.toLocaleString('id-ID');
        
        if(document.getElementById('portfolio-total-estimasi')) document.getElementById('portfolio-total-estimasi').innerText = "Rp " + totalEstimasiSeluruhnya.toLocaleString('id-ID');
...
// Fungsi untuk menghitung total semua saldo user dari DB2 (Database Utama) secara realtime
function hitungTotalSaldoGlobalRealtime() {
    try {
        // Pengecekan apakah database utama (db2) sudah siap digunakan
        if (typeof db2 === 'undefined' || !db2 || typeof db2.ref !== 'function') {
            console.warn("[Global Supply] Database DB2 belum siap, mencoba ulang dalam 1 detik...");
            setTimeout(hitungTotalSaldoGlobalRealtime, 1000);
            return;
        }

        console.log("[Global Supply] Melacak saldo global dari DB2 aktif...");

        // Menembak node 'cube_users' langsung menggunakan 'db2' utama Anda
        db2.ref("cube_users").on("value", (snapshot) => {
            const balanceDisplayNode = document.getElementById("global-users-total-balance");
            
            if (!balanceDisplayNode) return; // Keluar jika elemen HTML belum dirender di layar

            let totalSaldoSemuaUser = 0;

            if (snapshot.exists()) {
                // Looping memeriksa akun satu per satu dari database kedua
                snapshot.forEach((childSnapshot) => {
                    try {
                        const userData = childSnapshot.val();
                        if (!userData) return; // Lewati jika data kosong/null

                        // MENGGUNAKAN FIELD DATA UTAMA SCRIPT ANDA: userData.balance
                        if (typeof userData.balance !== 'undefined') {
                            let saldoUser = parseInt(userData.balance);
                            
                            // Pastikan data adalah angka yang valid (bukan teks rusak / NaN)
                            if (!isNaN(saldoUser)) {
                                totalSaldoSemuaUser += saldoUser;
                            }
                        }
                    } catch (loopError) {
                        // Jika ada satu user datanya korup, bypass agar hitungan user lain tetap jalan
                        console.error("[Global Supply] Error baris user:", loopError);
                    }
                });

                // Tampilkan hasil akumulasi semua saldo asli user ke layar HTML
                balanceDisplayNode.innerHTML = `Rp ${totalSaldoSemuaUser.toLocaleString('id-ID')}`;
                console.log("[Global Supply] Total Terhitung dari DB2: Rp", totalSaldoSemuaUser);
            } else {
                balanceDisplayNode.innerHTML = "Rp 0";
            }
        }, (error) => {
            console.error("[Global Supply] Firebase DB2 menolak akses baca:", error);
        });

    } catch (err) {
        console.error("[Global Supply] Sistem crash total:", err);
    }
}

// Jalankan fungsi secara otomatis mengikuti lifecycle DOM browser
